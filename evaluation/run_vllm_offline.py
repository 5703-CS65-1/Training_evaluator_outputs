"""Offline vLLM evaluation pipeline.

Stage A: vLLM offline batch inference — single in-process LLM, all prompts
         submitted at once, continuous-batched by vLLM.
Stage B/C/D: reuse `eval_pipeline` (claim extraction → judging → metrics)
         with candidate texts already in hand.

不起 OpenAI server。Stage A 跑完后 candidate texts 全部落盘
`outputs/candidate_texts.jsonl`，stage B/C/D 全部异步并发。

用法:
  bash run_vllm_offline.sh
或直接:
  /root/autodl-fs/conda_envs/vllm/bin/python run_vllm_offline.py \\
      --model-path /root/autodl-fs/models/qwen3vl-8b-aesthetic-merged \\
      --data data/eval_text/1.jsonl \\
      --image-dir data/eval_images/package_1_images \\
      --output outputs/qwen3vl-8b-vllm-offline \\
      --judge-api-key $DASHSCOPE_API_KEY \\
      --judge-base-url https://dashscope-intl.aliyuncs.com/compatible-mode/v1 \\
...
from vllm_offline import VllmCandidateEngine
...
def compute_and_save_score_metrics(
    ...
    *,
    ...
    """Compute MAE and Spearman from Stage A results and gold scores.

    Idempotent: if `score_metrics.json` already exists and its `num_scored`
    matches the current candidate set, skip recompute unless `force=True`.
    """
...
# ───────────────────────── Stage A: offline vLLM batch ────────────────────────
def _load_existing_candidates(path: Path) -> dict[str, CandidateOutput]:
    """Read previously-written candidate_texts.jsonl, return {sample_id: CandidateOutput}."""
...
def run_stage_a_vllm(
    samples: list[GoldSample],
    image_dir: Path,
    prompt: str,
    engine_factory,           # callable -> VllmCandidateEngine, lazy so resume w/ 0 todo skips loading
    output_dir: Path,
    resume: bool,
    ...
    if items:
        engine = engine_factory()  # only load vLLM if we actually have work
        ...
        # Append new candidates to the JSONL (preserve existing lines on resume).
        mode = "a" if resume and cand_path.exists() else "w"
        with cand_path.open(mode, encoding="utf-8") as fh:
            # If we're starting fresh (not resuming), also rewrite existing-resumed
            # entries (which is empty anyway in the not-resume branch).
            if mode == "w":
...
def run_stage_b(
    ...
    """Extract candidate claims for every sample. 本地无 API，幂等。

    已有 claims 的 sample 直接复用，只对缺失样本抽取并 append。
    全部已抽过 → short-circuit，不触碰 jsonl。
    """
    output_dir.mkdir(parents=True, exist_ok=True)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="ArtcomBench offline vLLM eval")
    ...
    # Sampling — Qwen3 thinking defaults
    ...
    p.add_argument("--enable-thinking", action="store_true",
                   help="Pass enable_thinking=True to judge model (e.g. Qwen3 series).")
    ...
    p.add_argument("--stage-a-only", action="store_true",
                   help="Run only Stage A (vLLM batch); skip Stage B/C/D.")
    p.add_argument("--resume", action="store_true",
                   help="Skip samples already present in candidate_texts.jsonl / judged_results.jsonl.")
...
def main() -> None:
    ...
    # Judge key only required when we'll actually run Stage C
    if not args.stage_a_only and not config.JUDGE_API_KEY:
        raise SystemExit(
            "[ERROR] judge api key is empty — pass --judge-api-key, set "
            "OPENAI_API_KEY / DASHSCOPE_API_KEY, or run with --stage-a-only"
        )
    ...
    # ── Stage A: vLLM offline batch (with resume support)
    t_a = time.time()

    def _make_engine() -> VllmCandidateEngine:
        return VllmCandidateEngine(
    ...
    candidates, num_new_candidates = run_stage_a_vllm(
    ...
    # ── Score metrics: only recompute when Stage A produced new candidates (or file missing)
